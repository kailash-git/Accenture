...
async function selectScenario(scenarioKey) {
  ...
  if (headlineEl) {
    headlineEl.innerHTML = anom.headline;
  ...
  const regionLabel = STATE_TO_REGION[anom.region] || anom.region || '';
  if (titleEl) titleEl.textContent = `${anom.sku} · ${regionLabel} · ${anom.title}`;
  ...
  if (actionCardsGrid) {
    if (anom.abstained) {
      ...
      const fields = [
        ...
        { label: 'Action', val: a.title },
        { label: 'Expected Impact', val: a.expectedImpact },
        { label: 'Owner', val: a.owner },
        { label: 'Confidence', val: `${Math.round(a.confidence)}%` },
      ...
      const btnHtml = isApproved
        ...
        : `<button class="btn-action-primary" onclick="handleActionApprove('${scenarioKey}', -1, this)">Approve Action</button>`;

      actionCardsGrid.innerHTML = `
        <div class="action-module-card" id="actionCard-${scenarioKey}" style="grid-column: 1 / -1;">
          <div class="structured-action-schema-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; margin-bottom: 14px;">
            ${fields.map(f => `
              <div>
                <div style="font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-tertiary); margin-bottom: 4px;">${f.label}</div>
                <div style="font-size: 13px; color: var(--text-primary); line-height: 1.4;">${f.val}</div>
              </div>
            `).join('')}
          </div>
...
function _layoutGraphNodes(nodes) {
  ...
  // cx/cy/radius are tuned against the SVG's own viewBox (600x460, see the
  // template below) so that even a full 3-hop ring (radius 205 here, at any
  // angle including straight up/down) stays inside the box with margin to
  // spare -- the previous radius (60 + hop*65 = 255 at hop 3, against a
  // 380-tall box centered at cy=190) could place a hop-3 node ~65px above
  // the box's top edge, and with the SVG's overflow:visible that node
  // visually collided with the explainer text sitting directly above it.
  const cx = 300, cy = 230;
  const positions = {};
  Object.keys(byHop).sort((a, b) => a - b).forEach(hopStr => {
    const hop = Number(hopStr);
    const group = byHop[hop];
    const radius = hop === 0 ? 0 : 55 + hop * 50;
    group.forEach((n, i) => {
      if (radius === 0) {
...
function _kgExplainerHtml() {
  return `<div class="kg-explainer">
    <strong>How this works:</strong> traverses outward from this anomaly's item and region nodes (bold ring)
    ...
    can't be pulled in as if it explained this one.
  </div>`;
}
...
function renderKnowledgeGraphPanel(graphData, itemId, anom) {
  ...
  if (!wrap) return;
  if (detail) detail.innerHTML = `<div class="kg-detail-placeholder">Click any node to inspect the retrieved record behind it.</div>`;
  ...
  const pos = _layoutGraphNodes(graph.nodes);
  const nodeById = {};
  ...
  const nodeSvg = graph.nodes.map((n, i) => {
    ...
    const label = restricted ? 'RESTRICTED' : _truncateLabel(n.label);
    ...
    return `<g class="kg-node ${isOrigin ? 'kg-node-origin' : ''} ${restricted ? 'kg-node-restricted' : ''}"
      ...
      <text x="${p.x}" y="${p.y + labelDy}" class="kg-node-label" fill="${restricted ? 'var(--accent-red)' : 'var(--text-secondary)'}">${_escapeHtml(label)}</text>
    </g>`;
  }).join('');
  ...
  wrap.innerHTML = `
    ...
    ${_kgExplainerHtml()}
    <div class="kg-graph-meta">${graph.nodes.length} nodes · ${(graph.edges || []).length} edges within 3 hops · server-masked per role</div>
    <svg class="kg-graph-svg" viewBox="0 0 600 460" preserveAspectRatio="xMidYMid meet">
      ${edgeSvg}
      ${nodeSvg}
    </svg>
