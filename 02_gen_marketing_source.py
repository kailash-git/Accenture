...
for state, region_name in STATE_TO_REGION_NAME.items():
    ...
    for wk in weeks:
        ...
        month = wk.month
        year = wk.year
        years_elapsed = year - weeks[0].year

        for ch in CHANNELS:
            base_spend = {"Digital": 800, "InStore_Promo": 500, "TV": 1200}[ch]

            # --- deterministic structure (a real marketing calendar has
            # planned, explainable seasonal patterns, not just noise) ---
            # holiday ramp: Nov-Dec spend rises, peaking in December,
            # mirroring a real retailer's actual budget calendar
            if month == 12:
                seasonal_mult = 1.45
            elif month == 11:
                seasonal_mult = 1.25
            elif month in (1, 2):
                seasonal_mult = 0.85   # post-holiday pullback
            else:
                seasonal_mult = 1.0

            # back-to-school bump in Aug, InStore_Promo and Digital only
            if month == 8 and ch in ("Digital", "InStore_Promo"):
                seasonal_mult *= 1.15

            # mild year-over-year budget growth, a deterministic trend,
            # not a random walk, real ad budgets tend to grow with the business
            trend_mult = 1 + 0.04 * years_elapsed

            # demand responsiveness stays as before: a noisy, non-deterministic
            # link to actual sales, so spend isn't ONLY a fixed calendar formula
            demand_factor = 1 + 0.15 * (base_demand / max(state_rev.mean(), 1) - 1)
            noise = rng.normal(1.0, 0.08)   # tightened slightly now that
                                             # real structure carries more of the variation

            spend = max(0, base_spend * seasonal_mult * trend_mult * demand_factor * noise)
            rows.append({
