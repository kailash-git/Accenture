...
for _, r in sku_lookup.iterrows():
    for state in states:
        for m in months:
            # organic random variation, not a flat baseline:
            # most months have 0 stockout days, occasionally 1-2 happen
            # naturally (a Poisson process with low rate mimics real,
            # infrequent supply hiccups rather than a suspiciously flat zero)
            organic_stockout_days = min(int(rng.poisson(0.25)), 3)
            # fill rate responds to stockouts, plus its own independent noise.
            # A hard clip at 1.0 was tried first but flattened ~3.4% of rows
            # to an identical, suspiciously round 1.0000 (a real fill rate
            # essentially never lands on exactly 100.00%). Reflect draws that
            # exceed 1.0 back below it instead of clipping, so the upper tail
            # stays continuous and varied rather than piling up on one value.
            raw = rng.normal(0.985, 0.01) - organic_stockout_days * 0.02
            if raw > 1.0:
                raw = 1.0 - (raw - 1.0)   # reflect the overshoot back down
            # only a lower clip is needed now - reflection already keeps the
            # upper side below 1.0 without needing a second hard ceiling,
            # which would just recreate the same pileup problem at a new value
            fill_rate = max(raw, 0.85)
            rows.append({
                "warehouse_sku": r.warehouse_sku,
                "state_id": state,
                "month": m,
                "fill_rate": round(fill_rate, 4),
                "stockout_days": organic_stockout_days,
            })

supply = pd.DataFrame(rows)
print(f"Organic stockout distribution across all {len(supply)} sku-state-months "
      f"(before the scripted injection below):")
print(supply.stockout_days.value_counts().sort_index())
