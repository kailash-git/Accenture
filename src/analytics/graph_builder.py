...
from src.analytics.series_anomaly import detect_trailing_zscore, detect_pct_change, detect_supply_rule
from src.analytics.sentiment import get_feedback_monthly_sentiment
from src.analytics.graph_entities import build_entity_graph, attach_belongs_to


def add_sales_anomaly_nodes(graph, db_path, window=8, z_threshold=1.1, price_threshold=0.03):
    """
    Adds sales_anomaly nodes (units, revenue, sell_price) computed on the
    item x state x day aggregate, and wires each into its item/state entity
    nodes via belongs_to edges.
    ...
    for _, row in units.iterrows():
        _add_sales_node(graph, row, 'units', row['z'])
    for _, row in revenue.iterrows():
        _add_sales_node(graph, row, 'revenue', row['z'])
    for _, row in price.iterrows():
        _add_sales_node(graph, row, 'sell_price', row['pct_chg'])

    return graph


def _add_sales_node(graph, row, column, score):
...
def add_temporal_edges(graph):
    ...
    daily_nodes = sales_nodes + event_nodes

    sales_by_date = defaultdict(list)
    for n, a in sales_nodes:
